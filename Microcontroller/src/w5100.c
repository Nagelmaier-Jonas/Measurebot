...
static uint16 wiz_read_buf(uint16 addr, uint8* buf,uint16 len);
...
static uint8 mysockets[MAX_SOCK_NUM] = {0xFF,0xFF,0xFF,0xFF};
...
uint8 IINCHIP_WRITE(uint16 addr,uint8 data)
{
  ...
  IINCHIP_SpiSendData(W5100_WRITE_OPCODE);
...
uint8 IINCHIP_READ(uint16 addr)
{
	uint8 data;
  IINCHIP_CSoff();                             // CS=0, SPI start
  IINCHIP_SpiSendData(W5100_READ_OPCODE);
  IINCHIP_SpiSendData((addr & 0xFF00) >> 8);
  IINCHIP_SpiSendData(addr & 0x00FF);
...
#ifdef __DEF_IINCHIP_DBG__
    printf("Sock %d : %.4x : %.4x : %.4x : %.4x\r\n", i, (uint16) SBUFBASEADDRESS[i],
        (uint16) RBUFBASEADDRESS[i], SSIZE[i], RSIZE[i]);
#endif
  }
}
...
void W5100GetGWIPAddr(uint8 * addr)
{
	...
	)
{
	...
	SUBN_VAR[2] = addr[2];
	SUBN_VAR[3] = addr[3];
	W5100ApplySubNetMask();
}
void W5100GetSubNetMask(uint8 * addr)
{
  addr[0] = SUBN_VAR[0];
  addr[1] = SUBN_VAR[1];
  addr[2] = SUBN_VAR[2];
...
@brief	It sets up SubnetMask address
*/ 
void W5100ClearSubAddr(
	void	/**< a pointer to a 4 -byte array responsible to set the SubnetMask address */
	)
{
	
  W5100GetIPAddr(IP_VAR);
...
void W5100GetMACAddr(uint8 * addr)
{
...
void W5100GetIPAddr(uint8 * addr)
{
...
*/
uint16 W5100GetSn_RX_RSR(SOCKET s)
{
	uint16 val=0,val1=0;
	do
	{
